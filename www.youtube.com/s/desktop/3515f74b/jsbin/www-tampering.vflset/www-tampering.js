...
function c(f,d){var g=f.g;return u(function(){if(g){for(;g.head!=f.g;)g=g.previous;for(;g.next!=g.head;)return g=g.next,{done:!1,value:d(g)};g=null}return{done:!0,value:void 0}})}
function e(f,d){var g=d&&typeof d;"object"==g||"function"==g?l.has(d)?g=l.get(d):(g=""+ ++m,l.set(d,g)):g="p_"+d;var k=f.data_[g];if(k&&y(f.data_,g))for(f=0;f<k.length;f++){var v=k[f];if(d!==d&&v.key!==v.key||d===v.key)return{id:g,list:k,index:f,h:v}}return{id:g,list:k,index:-1,h:void 0}}
function h(f){this.data_={};this.g=b();this.size=0;if(f){f=w(f);for(var d;!(d=f.next()).done;)d=d.value,this.set(d[0],d[1])}}
...
var l=new WeakMap;h.prototype.set=function(f,d){f=0===f?0:f;var g=e(this,f);g.list||(g.list=this.data_[g.id]=[]);g.h?g.h.value=d:(g.h={next:this.g,previous:this.g.previous,head:this.g,key:f,value:d},g.list.push(g.h),this.g.previous.next=g.h,this.g.previous=g.h,this.size++);return this};
h.prototype.delete=function(f){f=e(this,f);return f.h&&f.list?(f.list.splice(f.index,1),f.list.length||delete this.data_[f.id],f.h.previous.next=f.h.next,f.h.next.previous=f.h.previous,f.h.head=null,this.size--,!0):!1};
